...
def save_config(config, path='config_out.json'):
    tf.print(f"[config_handler.save_config] Preparing to save configuration to: {path}")
    config_composed = compose_config(config) # Selects which config items to save
    tf.print(f"[config_handler.save_config] Config items after compose_config (keys): {list(config_composed.keys())}")

    # CRITICAL FIX: Use sanitize_dict_for_json on the composed config
    config_sanitized_for_save = sanitize_dict_for_json(config_composed)
    tf.print(f"[config_handler.save_config] Config items after sanitize_dict_for_json.")
    ...
    try:
        ...
        if dir_name: # Check if path includes a directory
            ...
            tf.print(f"[config_handler.save_config] Ensured directory exists: {dir_name}")

        with open(path, 'w') as f:
            json.dump(config_sanitized_for_save, f, indent=4, cls=NpEncoder) # NpEncoder for datetime etc.
        tf.print(f"[config_handler.save_config] Composed and SANITIZED configuration saved successfully to {path}")
    except Exception as e:
        tf.print(f"[config_handler.save_config] ERROR saving configuration to {path}: {e}")
