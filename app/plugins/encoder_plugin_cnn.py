    ...
    def configure_size(self, input_shape, interface_size):
       ...
       # add conv and maxpooling layers, calculating their kernel and pool sizes
        ...
        for size in layers:
            ...
            x = Conv1D(filters=size, kernel_size=kernel_size, activation='leaky_relu ', kernel_initializer=HeNormal(), kernel_regularizer=l2(0.01), padding='same')(x)
        ...
        outputs = Dense(interface_size, activation='leaky_relu', kernel_initializer=GlorotUniform(), kernel_regularizer=l2(0.01))(x)
