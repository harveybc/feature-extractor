# fe_pretrainer_gpu: Trains a dcn with the actions per symbol as output, after the 
# training, exports the features from a bottleneck layer in a separate csv file 
# to be used to neuroevolve a network that controls an agent, (previously tested working strategy with ideal signals)
...
class QPretrainer():    
    ...
    def set_dcn_model_r(self):
        ...
        model.add(Conv1D(256, 3,  activation='relu',  use_bias=False)) 
        model.add(BatchNormalization()) 
        ...
        # TODO: CAMBIAR PARA 25 outputs (5*num_symbols)
                
        # Output Layers
        # Adds 15 outputs for regresssion
        r_out = Dense(15, activation = 'linear') (model)
        # Add 10 outputs for classification
        c_out = Dense(10, activation = 'linear') (model)
    ...
    ## Train SVMs with the training dataset using c ross-validation error estimation
    ...
    def train_model(self, signal):
        ...
        fig.savefig('nbn_predict_' + str(signal) + '_mae.png', dpi=600)
        ...
        plt.xlabel('epoch')
        plt.legend(['train', 'test'], loc='upper left')
        fig.savefig('nbn_predict_' + str(signal) + '_loss.png', dpi=600)
        return self.svr_rbf 
    ...
    def evaluate_validation(self, params, signal):
        ...
        with open('output_act_'+ str(signal) +'.csv' , 'w', newline='') as myfile:
            wr = csv.writer(myfile)
            wr.writerows(y_rbf)
        ...
        plt.legend()
        fig.savefig('nbn_predict_' + str(signal) + '.png', dpi=1000)
