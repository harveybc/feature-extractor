...
class Plugin:
    ...
    def configure_size(self, input_shape, interface_size):
        ...
        for size in layers[1:-1]:  # Use the layers except the first and the last one
            ...
            x = Conv1D(filters=size, kernel_size=kernel_size, activation='linear', kernel_initializer=GlorotUniform(), kernel_regularizer=l2(0.01), padding='same')(x)
        ...
        outputs = x = Conv1D(filters=interface_size, kernel_size=kernel_size, activation='linear', kernel_initializer=GlorotUniform(), kernel_regularizer=l2(0.01), padding='same')(x)
