...
class Plugin:
    ...
    def configure_size(self, interface_size, input_shape, num_channels=None, encoder_output_shape=None, use_sliding_windows=False):
        """
        Configures the LSTM decoder model to mirror the encoder's architecture.
        
        Args:
            interface_size (int): Size of the latent vector (decoder input).
            input_shape (int): Output sequence length (number of time steps to reconstruct).
            num_channels (int, optional): Number of features per time step (default=1).
            encoder_output_shape: Not used in this configuration.
            use_sliding_windows (bool): Ignored here; decoder always expects a 2D latent vector.
        """
        self.params['interface_size'] = interface_size
        self.params['output_shape'] = input_shape  # sequence length to reconstruct
        if num_channels is None:
            num_channels = 1

        # Compute encoder layer sizes as done in the encoder
        ...
        # For the decoder, mirror the encoder LSTM sizes (exclude the latent layer)
        ...
        # Expand latent vector to match the size of the last LSTM unit in the encoder (penultimate encoder layer)
        latent_dense_units = encoder_layers[-2]
        self.model.add(Dense(latent_dense_units,
        ...
        if dropout_rate > 0:
            self.model.add(Dropout(dropout_rate, name="decoder_dropout_after_expand"))
        # Repeat the expanded vector to form a sequence
        self.model.add(RepeatVector(input_shape))
        print(f"[configure_size] Added RepeatVector with output length: {input_shape}")
        ...
        for idx, units in enumerate(decoder_lstm_sizes, start=1):
            ...
            if dropout_rate > 0:
                self.model.add(Dropout(dropout_rate, name=f"decoder_dropout_{idx}"))
        # Final TimeDistributed Dense layer to reconstruct original features per time step
        self.model.add(TimeDistributed(Dense(num_channels,
                                             activation='linear',
                                             kernel_initializer=GlorotUniform(),
                                             kernel_regularizer=l2(self.params.get('l2_reg', 1e-2))),
                                       name="decoder_output"))
        self.model.compile(optimizer=Adam(learning_rate=self.params.get('learning_rate', 0.0001),
