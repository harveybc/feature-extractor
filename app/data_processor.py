...
def run_autoencoder_pipeline(config, encoder_plugin, decoder_plugin):
    ...
    if not config.get('use_sliding_windows', True):
        if encoder_plugin_name in ['lstm', 'transformer']:
            ...
            # (i.e. each sample's original feature vector becomes a sequence of length = num_features with 1 channel)
            ...
            # Set original_feature_size to the number of time steps (i.e. the original feature count)
    ...
    # - In non-sliding mode:
    #   • For LSTM/Transformer: input_size equals the original_feature_size.
